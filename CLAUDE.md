...
- `npx astro sync` — regenerate Astro's generated types, including the `astro:env` virtual module (`.astro/types.d.ts`). Run after `npm install` and after editing the `env.schema` in `astro.config.mjs`. **Type-checked lint and the build fail with unresolved-import / missing-type errors if this hasn't run** — CI runs `npx astro sync` before `npm run lint` for exactly this reason (see `.github/workflows/ci.yml`).

No test runner is configured: `lint` + `build` are the only validation gates. `@astrojs/check` is installed but not wired to a script — run `npx astro check` for a one-off type check.
