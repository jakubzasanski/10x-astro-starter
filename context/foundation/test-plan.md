...
| 2   | API integration + access control               | Auth gate, validation/413, two-user RLS isolation, and correct owner/origin on `/api/generate` and `/api/cards`                              | #1, #4, #5, #6 | integration     | complete    | context/changes/testing-api-integration-access-control/ |
...
| integration data       | local Supabase       | n/a     | shipped §3 Phase 2. `npx supabase start` (Docker). Two real users for the RLS isolation test (`npm run test:integration`, separate Vitest project so the Docker-free `npm test` floor is unaffected) — see §6.4.                                                                                                                                               |
...
| integration (API + RLS)                         | CI on PR             | required after §3 Phase 2 (tests shipped; CI wiring is §3 Phase 4) | auth/ownership regressions, save persistence   |
...
- **Layer split (load-bearing)**: handler-level properties (auth gate, body cap, validation, owner/origin) are proven **without a database** in the Vitest **unit** project; true cross-user RLS isolation is a separate **integration** test against local Supabase (see §6.4). Two Vitest projects do the split — `npm test` runs unit only (Docker-free), `npm run test:integration` runs the DB suite. Don't push handler-property assertions into the DB layer, or DB isolation into a mocked handler test.
- **Invoke the handler directly**: import `POST` from the route and call it with a synthetic context from `makeApiContext()` (`test/support/api-context.ts`). It builds only the slice the handlers read (`locals.user`, `request.headers`, `request.json()`, `cookies`). **Build `request` as a plain object, not a real `Request`** — undici recomputes `content-length` on a real Request body, which breaks the 413 test that sets an arbitrary oversized header.
- **413 fires only on a present, numeric, oversized `content-length`**: `Number(null)=0`, `Number("abc")=NaN` both fall through to parsing — set a numeric oversized header to hit the cap.
- **Mock the Supabase client, capture the insert**: `vi.mock("@/lib/supabase")` so `createClient` returns a fake whose `from("flashcards").insert(rows)` records its argument. Assert the route forces `source:"ai"` + `user_id = locals.user.id` and that a client-supplied `user_id`/`source` never survives (zod strips unknown keys at parse; the map overwrites). Reset call history with `vi.clearAllMocks()` in `afterEach` — `restoreAllMocks()` does NOT clear `vi.fn()` call counts, so a prior test's call leaks into `not.toHaveBeenCalled()` assertions.
- **Generic error bodies**: assert a sentinel fed through `sourceText` / a card's question never appears in a 5xx/502 body (privacy, risk #2).
- **Oracle**: assert status codes + the persisted payload against the PRD contract, not card JSON copied from the route. Use boundary values (`MAX_SOURCE_CHARS` / `MAX_CANDIDATES` pass; `+1` fails). Break-verify each test (invert the protected behavior → red → revert).
- **Reference tests**: `test/handlers/generate.test.ts`, `test/handlers/cards.test.ts`.
...
- **Server (load-bearing)**: tests drive a **production build** via `npm run build && npm run preview`, **not** `astro dev`. The dev server intermittently streams empty (0-byte) auth pages (SSR wedging) and injects a click-intercepting dev toolbar; the preview build is deterministic. `reuseExistingServer: false` (locally and in CI) so every run builds the current code — reusing a stale `astro preview` (serving its old `dist/`) silently ran the suite against outdated code. Playwright owns the server lifecycle and frees the port on exit; if `:4321` is occupied it fails fast.
...
- **Two real users, asserted through their OWN tokens**: mint user A and user B via the GoTrue admin API (`ensureTestUser`, service-role) — seeding ONLY — then `getUserToken(email, password)` per user and hit PostgREST (`flashcardsRequest`) with each user's bearer token + the anon apikey. **Never assert isolation through the service-role key**: RLS on `flashcards` is `enable`, not `force`, so service-role bypasses every policy and the assertion would prove nothing (`lessons.md`).
- **What to assert**: B sees 0 of A's rows (SELECT); B's UPDATE/DELETE of A's row affects 0 rows (the `using` filter — returns `[]` with `Prefer: return=representation`, not a 403); B's INSERT with `user_id = A` is rejected (the `with check` clause → 403); A retains full access to A's own row. Get A's uuid for seeding/forging from the JWT `sub` claim (`userIdFromToken`).
- **Isolation + re-run safety**: tag each row's question with a unique `runId` (`rls-${Date.now()}`); clean up in `afterAll` via each owner's own token (`deleteCardsByQuestionPrefix(runId, email, password)`) — `service_role` has no GRANT on `flashcards`. Verified re-run-safe back-to-back.
- **Break-verify**: temporarily loosen a policy on the local DB (e.g. `alter policy "flashcards_select_own" … using (true)` via `docker exec supabase_db_<project> psql`), confirm the relevant assertion goes red, then revert.
- **Run**: needs `npx supabase start` (Docker); `npm run test:integration` (the integration Vitest project — kept out of the Docker-free `npm test`).
- **Reference test**: `test/integration/rls-flashcards.test.ts`. Shared helpers: `test/support/supabase.ts`.
...
- **Phase 2 (API integration + access control)** — shipped 2026-06-21. Two test layers behind a Vitest unit/integration project split (so `npm test` stays Docker-free; `npm run test:integration` runs the DB suite). **Handler-property layer** (`test/handlers/{generate,cards}.test.ts`, unit project, no DB): auth gate 401 (#5), body-cap 413 + zod 400 incl. boundaries (#6), owner/origin forcing — `source:"ai"` + `user_id` = session user, client-supplied values stripped (#1 owner, #4 nothing-extra) — and generic error bodies with no `sourceText`/row leak (#2 reinforcement), via a synthetic `APIContext` + a mocked, insert-capturing Supabase client. **RLS isolation layer** (`test/integration/rls-flashcards.test.ts`, integration project, local Supabase): two real users asserted through their own PostgREST tokens — B sees/updates/deletes 0 of A's rows, B can't forge a row owned by A (with-check → 403), A keeps full access (#1 proper). Service-role mints users only, never asserts (RLS is `enable`, not `force`). All break-verified. Two-user helpers extracted to shared `test/support/` (e2e migrated to re-export shims). Also fixed e2e `reuseExistingServer` (was reusing a stale preview build). CI wiring deferred to Phase 4. Change: `context/changes/testing-api-integration-access-control/`.
